...
export default function Home() {
  ...
  const [airdropFilterStatus, setAirdropFilterStatus] = useState<"All" | "Completed" | "In Progress" | "Failed">("All");

  const filteredAirdropCampaigns = React.useMemo(() => {
    if (airdropFilterStatus === "All") return airdropCampaigns;
    return airdropCampaigns.filter((c) => c.status === airdropFilterStatus);
  }, [airdropCampaigns, airdropFilterStatus]);
                  ...
                  <div className="lg:col-span-6 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col gap-4" id="airdrop_history_card">
                    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-3">
                      <h3 className="text-sm font-bold text-white flex items-center gap-2">
                        ...
                        Campaign History ({filteredAirdropCampaigns.length})
                      </h3>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1.5 bg-slate-950 px-2 py-1 rounded-lg border border-slate-800 text-xs font-mono">
                          ...
                          <select
                            value={airdropFilterStatus}
                            onChange={(e) => setAirdropFilterStatus(e.target.value as "All" | "Completed" | "In Progress" | "Failed")}
                            className="bg-slate-900 text-slate-200 border border-slate-700/80 rounded px-2 py-0.5 text-xs font-mono focus:outline-none focus:border-purple-500 cursor-pointer"
                            id="airdrop_status_filter_dropdown"
                          >
                            <option value="All">All ({airdropCampaigns.length})</option>
                            <option value="In Progress">
                              In Progress ({airdropCampaigns.filter((c) => c.status === "In Progress").length})
                            </option>
                            <option value="Completed">
                              Completed ({airdropCampaigns.filter((c) => c.status === "Completed").length})
                            </option>
                            <option value="Failed">
                              Failed ({airdropCampaigns.filter((c) => c.status === "Failed").length})
                            </option>
                          </select>
                        </div>
                        ...
                        <button
                          onClick={() => {
                            const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(filteredAirdropCampaigns, null, 2));
                            const downloadAnchor = document.createElement("a");
                            downloadAnchor.setAttribute("href", dataStr);
                            downloadAnchor.setAttribute("download", `airdrop_campaigns_${airdropFilterStatus.toLowerCase()}_${Date.now()}.json`);
                            document.body.appendChild(downloadAnchor);
                            downloadAnchor.click();
                            downloadAnchor.remove();
                          }}
                          className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-[11px] font-mono transition-all flex items-center gap-1 cursor-pointer border border-slate-700/60"
                        >
                    ...
                    <div className="overflow-x-auto" id="airdrop_history_table_container">
                      <table className="w-full text-left border-collapse" id="airdrop_history_table">
                        ...
                        <tbody className="divide-y divide-slate-800/60 text-xs font-mono text-slate-300">
                          {filteredAirdropCampaigns.length === 0 ? (
                            <tr>
                              <td colSpan={6} className="py-8 text-center text-slate-500 font-mono text-xs">
                                No campaigns matching filter status &quot;{airdropFilterStatus}&quot;.
                              </td>
                            </tr>
                          ) : (
                            filteredAirdropCampaigns.map((campaign) => (
